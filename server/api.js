...
app.post('/login', function (req, res, next) {
    try {
        passport.authenticate('ldapauth', { session: false }, function (err, user, info) {
            ...
            const jwtToken = jwt.sign({ user: payload, avatar, token }, process.env.JWT_SECRET, {
                expiresIn: '6d',
            });
...
router.route('/consultasaldosctacte').get((request, response) => {
    ...
    const getData = getDataCliente ? { cliente: getDataCliente } : getDataFactura ? { factura: getDataFactura } : getDataQR ? { qr: getDataQR } : getDataNombre ? { nombre: getDataNombre } : null;
...
router.route('/consultasaldosctacteremito').get((request, response) => {
    ...
    const getDataFactura = request.query.numerofactura;
    const getDataQR = request.query.qr;
    const getData = getDataCliente ? { cliente: getDataCliente } : getDataFactura ? { factura: getDataFactura } : getDataQR ? { qr: getDataQR } : getDataNombre ? { nombre: getDataNombre } : getDataRemito ? { remito: getDataRemito } : null;
...
router.route('/gde/comprobantestockdelete/:id').delete(Pg.gde_comprobantestockDelete);
...
router.route('/gde/deposanoconsiderarupdate').post(Pg.gde_deposanoconsiderarCreate);
router.route('/gde/deposanoconsiderardelete/:id').delete(Pg.gde_deposanoconsiderarDelete);
...
router.route('/gde/recepcionproveedoraconsiderar').get(Pg.gde_recepcionproveedoraconsiderar);
router.route('/gde/recepcionproveedoraconsiderarupdate').post(Pg.gde_recepcionproveedoraconsiderarCreate);
router.route('/gde/recepcionproveedoraconsiderardelete/:id').delete(Pg.gde_recepcionproveedoraconsiderarDelete);
...
router.route('/enviarxWhatsapp').post(async (request, response) => {
    ...
    try {
        ...
        const filename = data.perfil === 'REA' ? process.env.PDF_FILENAME_REA : process.env.PDF_FILENAME_REB;
...
router.route('/recepcion-proveedores').get(async (request, response) => {
    try {
        ...
        if (fechaComprobanteDesde && fechaComprobanteHasta && fechaComprobanteDesde > fechaComprobanteHasta) {
            return response.status(400).json({
                ok: false,
