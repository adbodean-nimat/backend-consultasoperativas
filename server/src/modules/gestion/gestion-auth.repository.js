...
export async function syncAdUser(client, profile) {
  ...
  const result = await client.query(
    `INSERT INTO gf_usuarios (
     ...
     RETURNING *`,
    [username, profile.name ?? null, profile.displayName ?? null, profile.mail ?? null],
  );
...
export async function syncManagedAdUser(client, profile, active) {
  const username = normalizeSam(profile.sAMAccountName);
  const inserted = await client.query(
    `INSERT INTO gf_usuarios (
       sam_account_name, nombre, nombre_visible, email, activo
     )
     VALUES ($1, $2, $3, $4, $5)
     ON CONFLICT DO NOTHING
     RETURNING *`,
    [username, profile.name ?? null, profile.displayName ?? null, profile.mail ?? null, active],
  );
  if (inserted.rowCount) return { user: inserted.rows[0], created: true };

  const updated = await client.query(
    `UPDATE gf_usuarios
     SET nombre = $2, nombre_visible = $3, email = $4, activo = $5
     WHERE lower(sam_account_name) = $1
     RETURNING *`,
    [username, profile.name ?? null, profile.displayName ?? null, profile.mail ?? null, active],
  );
...
export async function findUserByIdForUpdate(client, id) {
  const result = await client.query(`SELECT * FROM gf_usuarios WHERE id = $1 FOR UPDATE`, [id]);
  return result.rows[0] ?? null;
}

export async function findUserByUsernameForUpdate(client, username) {
  const result = await client.query(
    `SELECT * FROM gf_usuarios WHERE lower(sam_account_name) = $1 FOR UPDATE`,
    [normalizeSam(username)],
  );
  return result.rows[0] ?? null;
}
...
export async function replaceUserRoles(client, userId, roleCodes, actorUserId) {
  ...
  await client.query(`DELETE FROM gf_usuario_roles WHERE usuario_id = $1`, [userId]);
