...
export async function syncOpenAIv2() {
  ...
  try {
    ...
    console.error(
      `No se encuentra el archivo local: ${LOCAL_FILE_PATH_PRODUCTOS}`,
    );
    process.exit(1);
  }
  try {
  ...
  console.log(
    "🔍 Buscando productos.json/faq.md/nimat_conocimiento_general.md existenten en el vector store...",
  );
  const existing = await findExistingFileByFilename(
    VECTOR_STORE_ID,
    TARGET_FILE_NAME_PRODUCTOS,
  );
  const existingFAQ = await findExistingFileByFilename(
    VECTOR_STORE_ID,
    TARGET_FILE_NAME_FAQ,
  );
  const existingINFO = await findExistingFileByFilename(
    VECTOR_STORE_ID,
    TARGET_FILE_NAME_INFO,
  );
  ...
  console.log(
    `Nuevo files subido. fileId: ${uploadedFile.id}, ${uploadedFileFAQ.id}, ${uploadedFileINFO.id}`,
  );
  ...
  if (
    ...
    existingFAQ?.vectorStoreFileId &&
    existingINFO?.vectorStoreFileId
  ) {
    ...
    await client.vectorStores.files.delete(existingFAQ.vectorStoreFileId, {
      vector_store_id: VECTOR_STORE_ID,
    });
    await client.vectorStores.files.delete(existingINFO.vectorStoreFileId, {
      vector_store_id: VECTOR_STORE_ID,
    });
