...
NP_A_Considerar AS (
    SELECT
        TCVE_TIPO_COM AS Cod_Comp,
        TCVE_NOMBRE AS Nomb_Comp
    FROM VENT_TCVE WITH (NOLOCK)
    WHERE TCVE_TIPO_COM IN ('NPC', 'NPG', 'NPT', 'PEA')
),
Compradores AS (
    SELECT
        u.USUA_USUARIO,
        g.GRUS_GRUPO,
        CASE u.USUA_USUARIO
            WHEN 'CAROLINA' THEN 'SCC'
            WHEN 'GESCOBAR' THEN 'EGE'
            WHEN 'LBORDOY'  THEN 'MLB'
            WHEN 'SARAUJO'  THEN 'SMA'
            ELSE NULL
        END AS Cod_Comp,
        CASE u.USUA_USUARIO
            WHEN 'CAROLINA' THEN 'Carolina'
            WHEN 'GESCOBAR' THEN 'Gabriel'
            WHEN 'LBORDOY'  THEN 'Laura'
            WHEN 'SARAUJO'  THEN 'Nino'
            ELSE NULL
        END AS Nomb_Comp
    FROM SEGU_USUA u WITH (NOLOCK)
    INNER JOIN SEGU_GRUS g WITH (NOLOCK)
        ON u.USUA_USUARIO = g.GRUS_USUARIO
    WHERE g.GRUS_GRUPO = 'COMPRA'
),
CTRL_AUTORIZ_NP AS (
    SELECT *
    FROM (VALUES
        (1, 'Límite de crédito total (A&F)'),
        (4, 'Días hist atraso pagos (A&F)'),
        (6, 'Destos modificados (VTAS)'),
        (7, 'Precios modificados (VTAS)')
    ) v(Cod_Crtl_Aut_NP, Nomb_Crtl_Aut_NP)
),
APROBACION_ESTADOS AS (
    SELECT *
    FROM (VALUES
        (1, 'Aprobado'),
        (2, 'Pendiente de aprobar'),
        (3, 'Desaprobado')
    ) v(Cod_Est_Aprob, Nomb_Est_Aprob)
),
ReservasNP AS (
    SELECT
        rd.RSDE_NUMINT_RSCA,
        rd.RSDE_DIVISION_NPCA,
        rd.RSDE_TIPO_NPCA,
        rd.RSDE_NUMERO_NPCA,
        rd.RSDE_RENGLON_NPDE,
        rd.RSDE_CANT_RESER,
        rc.RSCA_TIPO_RSCA,
        ru.RSCU_MOVSTO_MOST
    FROM VENT_RSDE rd WITH (NOLOCK)
    INNER JOIN VENT_RSCA rc WITH (NOLOCK)
        ON rd.RSDE_NUMINT_RSCA = rc.RSCA_NUMINT_RSCA
    LEFT JOIN VENT_RSCU ru WITH (NOLOCK)
        ON rd.RSDE_NUMINT_RSCA = ru.RSCU_NUMINT_RSCA
    WHERE ru.RSCU_MOVSTO_MOST IS NULL
),
NPAU_Pendientes AS (
    SELECT
        NPAU_DIVISION_NPCA,
        NPAU_TIPO_NPCA,
        NPAU_NUMERO_NPCA,
        MAX(CASE WHEN NPAU_CTRL_AUTORIZ = 1 THEN NPAU_CTRL_AUTORIZ END) AS Ctrl_Aut_1,
        MAX(CASE WHEN NPAU_CTRL_AUTORIZ = 4 THEN NPAU_CTRL_AUTORIZ END) AS Ctrl_Aut_4,
        MAX(CASE WHEN NPAU_CTRL_AUTORIZ = 6 THEN NPAU_CTRL_AUTORIZ END) AS Ctrl_Aut_6,
        MAX(CASE WHEN NPAU_CTRL_AUTORIZ = 7 THEN NPAU_CTRL_AUTORIZ END) AS Ctrl_Aut_7
    FROM VENT_NPAU WITH (NOLOCK)
    WHERE NPAU_ESTADO_APROB = 2
      AND NPAU_CTRL_AUTORIZ IN (1, 4, 6, 7)
    GROUP BY
        NPAU_DIVISION_NPCA,
        NPAU_TIPO_NPCA,
        NPAU_NUMERO_NPCA
...
Base1 AS (
    SELECT
        vend.VEND_VENDEDOR AS Cod_Vend,
        vend.VEND_NOMBRE AS Nomb_Vend,
        CAST(npca.NPCA_FECHA_EMI AS DATE) AS Fecha_Emi,

        npca.NPCA_CLIENTE AS Cod_Client,
        clie.CLIE_NOMBRE,
        clie.CLIE_CLASIF_6,
        npce.NPCE_NOMBRE,

        COALESCE(npce.NPCE_NOMBRE, clie.CLIE_NOMBRE) AS Nomb_Client,

        npde.NPDE_DIVISION_NPCA,
        npca.NPCA_TIPO_NPCA AS Tipo_NP,
        npca.NPCA_NUMERO_NPCA AS Nro_NP,
        npde.NPDE_RENGLON AS Renglon_NP,
        npde.NPDE_ARTICULO,

        arts.ARTS_ARTICULO_EMP,
        CASE 
            WHEN npca.NPCA_CLASIF_NPCV_1 = '-' THEN NULL
            ELSE arts.ARTS_ARTICULO_EMP
        END AS Cod_Art,

        arts.ARTS_NOMBRE,
        CASE 
            WHEN npca.NPCA_CLASIF_NPCV_1 = '-' THEN ' Falta CANAL VENTAS '
            ELSE arts.ARTS_NOMBRE
        END AS Nombre_Art,

        npde.NPDE_UNIMED AS Uni,

        npca.NPCA_FEC_ENT_BASE AS Fecha_Entr_Cabec,
        npde.NPDE_FECHA_ENTREGA AS Fecha_Entr_Item,

        CASE 
            WHEN npde.NPDE_FECHA_ENTREGA > DATEADD(DAY, 6, GETDATE()) THEN 'Verif. tiempo entrega'
            WHEN npca.NPCA_FEC_ENT_BASE <> npde.NPDE_FECHA_ENTREGA THEN 'Difer.fecha item<>fecha cabec'
            ELSE NULL
        END AS Verif_fecha_entr,

        CONVERT(DECIMAL(18,0), CASE WHEN npca.NPCA_CLASIF_NPCV_1 = '-' THEN NULL ELSE npde.NPDE_CANT_PEDIDA END) AS Cant_Ped,
        CONVERT(DECIMAL(18,0), CASE WHEN npca.NPCA_CLASIF_NPCV_1 = '-' THEN NULL ELSE npde.NPDE_CANT_FACTUR END) AS Cant_Fact,
        CONVERT(DECIMAL(18,0), CASE WHEN npca.NPCA_CLASIF_NPCV_1 = '-' THEN NULL ELSE npde.NPDE_CANT_ENTREG END) AS Cant_Entr,

        CONVERT(DECIMAL(18,0), CASE WHEN npca.NPCA_CLASIF_NPCV_1 = '-' THEN NULL ELSE npde.NPDE_CANT_PEDIDA - npde.NPDE_CANT_ENTREG END) AS A_Entregar,

        CONVERT(DECIMAL(18,0), CASE WHEN npca.NPCA_CLASIF_NPCV_1 = '-' THEN NULL ELSE ISNULL(res.RSDE_CANT_RESER, 0) END) AS Cant_Reserv,

        CONVERT(DECIMAL(18,0),
            CASE 
                WHEN npca.NPCA_CLASIF_NPCV_1 = '-' THEN NULL
                WHEN arts.ARTS_TIPO_ART IN ('011', '012') THEN 0
                ELSE (npde.NPDE_CANT_PEDIDA - npde.NPDE_CANT_ENTREG) - ISNULL(res.RSDE_CANT_RESER, 0)
            END
        ) AS Sin_Reser,

        res.RSCA_TIPO_RSCA AS Tipo_Reserv,

        CASE 
            WHEN res.RSCA_TIPO_RSCA = 'REO' THEN cpv2.CPV2_NOMBRE
            ELSE NULL
        END AS Listo_p_Entr,

        npca.NPCA_FLETE AS Cod_Tte_Cab_NP,
        npca.NPCA_OBSERVACION AS Observacion_NP,

        est.Nomb_Est_Aprob,
        npca.NPCA_OBSER_APROB,

        CASE 
            WHEN est.Nomb_Est_Aprob = 'Aprobado' THEN NULL
            ELSE npca.NPCA_OBSER_APROB
        END AS Observ_Aproba,

        ca1.Nomb_Crtl_Aut_NP AS Ctrl_Aut_1,
        CASE WHEN est.Nomb_Est_Aprob = 'Aprobado' THEN NULL ELSE ca1.Nomb_Crtl_Aut_NP END AS CtrlAut1,

        ca4.Nomb_Crtl_Aut_NP AS Ctrl_Aut_4,
        CASE WHEN est.Nomb_Est_Aprob = 'Aprobado' THEN NULL ELSE ca4.Nomb_Crtl_Aut_NP END AS CtrlAut4,

        ca6.Nomb_Crtl_Aut_NP AS Ctrl_Aut_6,
        CASE WHEN est.Nomb_Est_Aprob = 'Aprobado' THEN NULL ELSE ca6.Nomb_Crtl_Aut_NP END AS CtrlAut6,

        ca7.Nomb_Crtl_Aut_NP AS Ctrl_Aut_7,
        CASE WHEN est.Nomb_Est_Aprob = 'Aprobado' THEN NULL ELSE ca7.Nomb_Crtl_Aut_NP END AS CtrlAut7,

        npca.NPCA_CLASIF_NPCV_1,
        cpv1.CPV1_NOMBRE AS Nombre_Canal_Vta,

        arts.ARTS_TIPO_ART,
        npde.NPDE_CLASIF_NPDE_1,
        npca.NPCA_VEHICULO,
        npca.NPCA_RENGLON_DIR,
        clie.CLIE_ZONA_VENTA,

        arts.ARTS_PESO_EMB_UMS AS Kg_Art,
        arts.ARTS_CLASIF_2,
        arts.ARTS_CLASIF_6 AS Marca_Clasif6
    FROM VENT_NPCA npca WITH (NOLOCK)
    CROSS JOIN Parametros p
    INNER JOIN VENT_NPDE npde WITH (NOLOCK)
        ON npca.NPCA_DIVISION_NPCA = npde.NPDE_DIVISION_NPCA
       AND npca.NPCA_TIPO_NPCA     = npde.NPDE_TIPO_NPCA
       AND npca.NPCA_NUMERO_NPCA   = npde.NPDE_NUMERO_NPCA
    INNER JOIN NP_A_Considerar npt
        ON npde.NPDE_TIPO_NPCA = npt.Cod_Comp
    INNER JOIN VENT_NPVE npve WITH (NOLOCK)
        ON npve.NPVE_DIVISION_NPCA = npca.NPCA_DIVISION_NPCA
       AND npve.NPVE_TIPO_NPCA     = npca.NPCA_TIPO_NPCA
       AND npve.NPVE_NUMERO_NPCA   = npca.NPCA_NUMERO_NPCA
    INNER JOIN SIST_VEND vend WITH (NOLOCK)
        ON vend.VEND_VENDEDOR = npve.NPVE_VENDEDOR
    INNER JOIN STOC_ARTS arts WITH (NOLOCK)
        ON arts.ARTS_ARTICULO = npde.NPDE_ARTICULO
    INNER JOIN CCOB_CPV2 cpv2 WITH (NOLOCK)
        ON cpv2.CPV2_CLASIF_NPCV_2 = npca.NPCA_CLASIF_NPCV_2
    INNER JOIN CCOB_CLIE clie WITH (NOLOCK)
        ON clie.CLIE_CLIENTE = npca.NPCA_CLIENTE
    INNER JOIN CCOB_CPV1 cpv1 WITH (NOLOCK)
        ON cpv1.CPV1_CLASIF_NPCV_1 = npca.NPCA_CLASIF_NPCV_1
    INNER JOIN APROBACION_ESTADOS est
        ON est.Cod_Est_Aprob = npca.NPCA_ESTADO_APROB
    LEFT JOIN NPAU_Pendientes npau
        ON npau.NPAU_DIVISION_NPCA = npca.NPCA_DIVISION_NPCA
       AND npau.NPAU_TIPO_NPCA     = npca.NPCA_TIPO_NPCA
       AND npau.NPAU_NUMERO_NPCA   = npca.NPCA_NUMERO_NPCA
    LEFT JOIN CTRL_AUTORIZ_NP ca1
        ON ca1.Cod_Crtl_Aut_NP = npau.Ctrl_Aut_1
    LEFT JOIN CTRL_AUTORIZ_NP ca4
        ON ca4.Cod_Crtl_Aut_NP = npau.Ctrl_Aut_4
    LEFT JOIN CTRL_AUTORIZ_NP ca6
        ON ca6.Cod_Crtl_Aut_NP = npau.Ctrl_Aut_6
    LEFT JOIN CTRL_AUTORIZ_NP ca7
        ON ca7.Cod_Crtl_Aut_NP = npau.Ctrl_Aut_7
    LEFT JOIN VENT_NPCE npce WITH (NOLOCK)
        ON npce.NPCE_DIVISION_NPCA = npca.NPCA_DIVISION_NPCA
       AND npce.NPCE_TIPO_NPCA     = npca.NPCA_TIPO_NPCA
       AND npce.NPCE_NUMERO_NPCA   = npca.NPCA_NUMERO_NPCA
    LEFT JOIN ReservasNP res
        ON res.RSDE_DIVISION_NPCA = npde.NPDE_DIVISION_NPCA
       AND res.RSDE_TIPO_NPCA     = npde.NPDE_TIPO_NPCA
       AND res.RSDE_NUMERO_NPCA   = npde.NPDE_NUMERO_NPCA
       AND res.RSDE_RENGLON_NPDE  = npde.NPDE_RENGLON
    WHERE arts.ARTS_ARTICULO_EMP <> '99999999'
...
Base2 AS (
    SELECT
        ...
        b1.Listo_p_Entr,
        b1.Cod_Tte_Cab_NP,
        flet_item.FLET_NOMBRE AS Tte_Item,
        vehi.VEHI_NOMBRE AS Vehiculo,
        dcli.DCLI_DOMICILIO AS Domicilio,
        dcli.DCLI_LOCALIDAD AS Localidad,
        b1.Observacion_NP,
        b1.Nomb_Est_Aprob,
        b1.NPCA_OBSER_APROB,
        b1.Observ_Aproba,
        ...
        b1.Ctrl_Aut_7,
        b1.CtrlAut7,
        b1.NPCA_CLASIF_NPCV_1,
        b1.Nombre_Canal_Vta,
        b1.ARTS_TIPO_ART,
        b1.CLIE_ZONA_VENTA AS Zona_Vta,
        zven.ZVEN_NOMBRE AS Nombre_Zona_Vta,
        npca.NPCA_IMP_TOT_ORI,
        b1.Kg_Art,
        b1.ARTS_CLASIF_2,
        b1.Marca_Clasif6
    FROM Base1 b1
    INNER JOIN VENT_NPCA npca WITH (NOLOCK)
        ON b1.NPDE_DIVISION_NPCA = npca.NPCA_DIVISION_NPCA
       AND b1.Tipo_NP            = npca.NPCA_TIPO_NPCA
       AND b1.Nro_NP             = npca.NPCA_NUMERO_NPCA
    LEFT JOIN SIST_FLET flet_item WITH (NOLOCK)
        ON b1.NPDE_CLASIF_NPDE_1 = flet_item.FLET_FLETE
    LEFT JOIN DIST_VEHI vehi WITH (NOLOCK)
        ON b1.NPCA_VEHICULO = vehi.VEHI_VEHICULO
    LEFT JOIN CCOB_DCLI dcli WITH (NOLOCK)
        ON dcli.DCLI_CLIENTE = b1.Cod_Client
       AND dcli.DCLI_RENGLON = b1.NPCA_RENGLON_DIR
    LEFT JOIN SIST_ZVEN zven WITH (NOLOCK)
        ON zven.ZVEN_ZONA_VENTA = b1.CLIE_ZONA_VENTA
),
Base3 AS (
    SELECT
        b2.Cod_Vend,
        ...
        b2.Listo_p_Entr,
        b2.Cod_Tte_Cab_NP,
        flet_cab.FLET_NOMBRE AS Tte_Cab_NP,
        b2.Tte_Item,
        CASE 
            WHEN ISNULL(flet_cab.FLET_NOMBRE, '') <> ISNULL(b2.Tte_Item, '') THEN 'Verificar Transporte'
            ELSE NULL
        END AS Verif_Tte,
        b2.Vehiculo,
        b2.Domicilio,
        ...
        b2.Ctrl_Aut_7,
        b2.CtrlAut7,
        b2.NPCA_CLASIF_NPCV_1,
        b2.Nombre_Canal_Vta,
        b2.ARTS_TIPO_ART,
        b2.Zona_Vta,
        b2.Nombre_Zona_Vta,
        b2.NPCA_IMP_TOT_ORI,
        arco.ARCO_RUBRO_COMPRA,
        rubc.RUBC_NOMBRE,
        LEFT(rubc.RUBC_NOMBRE, 3) AS Cod_Compr,
        b2.Kg_Art,
        b2.ARTS_CLASIF_2,
        b2.Marca_Clasif6
    FROM Base2 b2
    INNER JOIN SIST_FLET flet_cab WITH (NOLOCK)
        ON b2.Cod_Tte_Cab_NP = flet_cab.FLET_FLETE
    LEFT JOIN STOC_ARCO arco WITH (NOLOCK)
        ON b2.NPDE_ARTICULO = arco.ARCO_ARTICULO
    LEFT JOIN CPAG_RUBC rubc WITH (NOLOCK)
        ON arco.ARCO_RUBRO_COMPRA = rubc.RUBC_RUBRO_COMPRA
),
Vincul1 AS (
    SELECT
        npve.NPVE_VENDEDOR AS Cod_Vend,
        vend.VEND_NOMBRE AS Nomb_Vend,
        CAST(npca.NPCA_FECHA_EMI AS DATE) AS Fecha_Emi,
        npca.NPCA_CLIENTE AS Cod_Client,
        COALESCE(npce.NPCE_NOMBRE, clie.CLIE_NOMBRE) AS Nomb_Client,
        npca.NPCA_DIVISION_NPCA,
        npca.NPCA_TIPO_NPCA AS Tipo_NP,
        npca.NPCA_NUMERO_NPCA AS Nro_NP,
        npde.NPDE_RENGLON AS Renglon_NP,
        arts.ARTS_ARTICULO_EMP AS Cod_Art,
        arts.ARTS_NOMBRE AS Nombre_Art,
        npde.NPDE_UNIMED AS Uni,
        npde.NPDE_FECHA_ENTREGA AS Fecha_Entr,
        CONVERT(DECIMAL(18,0), npde.NPDE_CANT_PEDIDA) AS Cant_Ped,
        CONVERT(DECIMAL(18,0), npde.NPDE_CANT_FACTUR) AS Cant_Fact,
        CONVERT(DECIMAL(18,0), npde.NPDE_CANT_ENTREG) AS Cant_Entr,
        CONVERT(DECIMAL(18,0), npde.NPDE_CANT_PEDIDA - npde.NPDE_CANT_ENTREG) AS A_Entregar
    FROM VENT_NPCA npca WITH (NOLOCK)
    CROSS JOIN Parametros p
    INNER JOIN VENT_NPVE npve WITH (NOLOCK)
        ON npca.NPCA_DIVISION_NPCA = npve.NPVE_DIVISION_NPCA
       AND npca.NPCA_TIPO_NPCA     = npve.NPVE_TIPO_NPCA
       AND npca.NPCA_NUMERO_NPCA   = npve.NPVE_NUMERO_NPCA
    INNER JOIN SIST_VEND vend WITH (NOLOCK)
        ON npve.NPVE_VENDEDOR = vend.VEND_VENDEDOR
    INNER JOIN CCOB_CLIE clie WITH (NOLOCK)
        ON clie.CLIE_CLIENTE = npca.NPCA_CLIENTE
    LEFT JOIN VENT_NPCE npce WITH (NOLOCK)
        ON npce.NPCE_DIVISION_NPCA = npca.NPCA_DIVISION_NPCA
    ...
    INNER JOIN VENT_NPDE npde WITH (NOLOCK)
       ...
       AND npca.NPCA_NUMERO_NPCA   = npde.NPDE_NUMERO_NPCA
    INNER JOIN STOC_ARTS arts WITH (NOLOCK)
        ON npde.NPDE_ARTICULO = arts.ARTS_ARTICULO
    INNER JOIN NP_A_Considerar npt
        ON npca.NPCA_TIPO_NPCA = npt.Cod_Comp
...
Vincul2 AS (
    SELECT
        ...
        v1.Cant_Entr,
        v1.A_Entregar,
        ocnp.OCNP_CANT_VINC AS Cant_Vinc_OC,
        rodc.RODC_FECHA_ENTREGA AS Fech_Prob_entr_Prov,
        rodc.RODC_CANT_PEDIDA AS Cant_OC,
        rodc.RODC_CANT_RECIB AS Cant_Recib_Prov,
        rodc.RODC_NUM_OC AS Nro_OC,
        prov.PROV_NOMBRE AS Proveedor,
        rubc.RUBC_NOMBRE,
        SUBSTRING(rubc.RUBC_NOMBRE, 1, 3) AS Cod_Comp
    FROM Vincul1 v1
    LEFT JOIN SIST_OCNP ocnp WITH (NOLOCK)
        ON v1.NPCA_DIVISION_NPCA = ocnp.OCNP_DIVISION_NPCA
       AND v1.Tipo_NP            = ocnp.OCNP_TIPO_NPCA
       AND v1.Nro_NP             = ocnp.OCNP_NUMERO_NPCA
       AND v1.Renglon_NP         = ocnp.OCNP_RENGLON_NPDE
    LEFT JOIN COMP_RODC rodc WITH (NOLOCK)
        ON ocnp.OCNP_DIVISION_CODC = rodc.RODC_DIVISION
       AND ocnp.OCNP_TIPO_CODC     = rodc.RODC_TIPO_OC
       AND ocnp.OCNP_NUMERO_CODC   = rodc.RODC_NUM_OC
       AND ocnp.OCNP_RENGLON_RODC  = rodc.RODC_REN_OC
    INNER JOIN STOC_ARTS arts WITH (NOLOCK)
        ON arts.ARTS_ARTICULO_EMP = v1.Cod_Art
    INNER JOIN STOC_ARCO arco WITH (NOLOCK)
        ON arts.ARTS_ARTICULO = arco.ARCO_ARTICULO
    INNER JOIN CPAG_RUBC rubc WITH (NOLOCK)
        ON rubc.RUBC_RUBRO_COMPRA = arco.ARCO_RUBRO_COMPRA
    LEFT JOIN COMP_CODC codc WITH (NOLOCK)
        ON rodc.RODC_DIVISION = codc.CODC_DIVISION
       AND rodc.RODC_TIPO_OC  = codc.CODC_TIPO_OC
       AND rodc.RODC_NUM_OC   = codc.CODC_NUM_OC
    LEFT JOIN CPAG_PROV prov WITH (NOLOCK)
        ON codc.CODC_PROVEEDOR = prov.PROV_PROVEEDOR
    WHERE ocnp.OCNP_CANT_VINC IS NOT NULL
),
Base4 AS (
    SELECT
        b3.Cod_Vend,
        ...
        b3.Marca_Clasif6
    FROM Base3 b3
    LEFT JOIN Vincul2 v2
        ON b3.NPDE_DIVISION_NPCA = v2.NPCA_DIVISION_NPCA
       AND b3.Tipo_NP            = v2.Tipo_NP
       AND b3.Nro_NP             = v2.Nro_NP
       AND b3.Renglon_NP         = v2.Renglon_NP
    LEFT JOIN Compradores comp
        ON b3.Cod_Compr = comp.Cod_Comp
),
MinimoFleteGratis AS (
    SELECT
        MAX(
            ROUND(
                (
                    CASE 
                        WHEN coti.COTI_COTIZACION IS NOT NULL 
                            THEN arpv.ARPV_PRECIO_VTA * coti.COTI_COTIZACION
                        ELSE arpv.ARPV_PRECIO_VTA
                    END
                ) * 0.8 * (1 + cimp.CIMP_TASA / 100.0),
                2
            )
        ) AS Monto_Min_Fle_Grtis
    FROM VENT_ARPV arpv WITH (NOLOCK)
    INNER JOIN STOC_ARTS arts WITH (NOLOCK)
        ON arpv.ARPV_ARTICULO = arts.ARTS_ARTICULO
    INNER JOIN VENT_ARVI arvi WITH (NOLOCK)
        ON arpv.ARPV_ARTICULO = arvi.ARVI_ARTICULO
    INNER JOIN SIST_CIMP cimp WITH (NOLOCK)
        ON arvi.ARVI_CATEGORIA_IMP = cimp.CIMP_CATEGORIA_IMP
       AND arvi.ARVI_IMPUESTO      = cimp.CIMP_IMPUESTO
    OUTER APPLY (
        SELECT TOP (1)
            c.COTI_COTIZACION
        FROM SIST_COTI c WITH (NOLOCK)
        WHERE c.COTI_MONEDA1 = arpv.ARPV_MONEDA
        ORDER BY c.COTI_FECHA DESC
    ) coti
    WHERE arpv.ARPV_LISTA_PRECVTA = 1
      AND arts.ARTS_ARTICULO_EMP = '01500025'
)
SELECT
    b4.Cod_Vend,
    b4.Nomb_Vend,
    ...
    b4.Cant_OC,
    b4.Cant_Recib_Prov,
    b4.Fech_Prob_entr_Prov,
    b4.Nro_OC,
    b4.Proveedor,
    b4.Nomb_Comp,

    mfg.Monto_Min_Fle_Grtis,

    CASE 
        WHEN b4.CLIE_CLASIF_6 = 'b'
         AND b4.NPCA_IMP_TOT_ORI < mfg.Monto_Min_Fle_Grtis
            THEN '$ bajo mínimo p/flete gratis. No se envía con camión Nimat'
        ELSE NULL
    END AS Solo_para_Reventa,

    b4.Kg_Art,

    CASE 
        WHEN b4.ARTS_TIPO_ART IN ('011', '012') 
            THEN ROUND(ISNULL(b4.A_Entregar, 0) * ISNULL(b4.Kg_Art, 0), 2)
        ELSE 
            ROUND(ISNULL(b4.Cant_Reserv, 0) * ISNULL(b4.Kg_Art, 0), 2)
    END AS Kg_Item_Reserv,

    b4.ARTS_CLASIF_2
FROM Base4 b4
CROSS JOIN MinimoFleteGratis mfg
ORDER BY
    b4.Fecha_Emi,
    b4.Nro_NP,
    b4.Renglon_NP;
